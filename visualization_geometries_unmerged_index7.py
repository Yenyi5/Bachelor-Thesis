...
map_path = r'D:/unmerged_geometries_index13.htm'
data = pd.read_csv(file_path,delimiter=';')

unmerged_map_path = r'D:\unmerged_geometries.htm'
...
um = folium.Map(location=[0, 0], zoom_start=2)

    
index_value = np.unique(data['index'].values)[10]    
mapping_data = data[data['index'] == index_value]

# Loop through each entry and add the merged geometry to the map
for index_value, unmerged_geometry in enumerate(mapping_data['geometry']):
    ...
    geo_json = mapping(unmerged_geometry)
...
#save unmerged map   
#um.save(unmerged_map_path)
#print(f"Saved unmerged map at {unmerged_map_path}")
