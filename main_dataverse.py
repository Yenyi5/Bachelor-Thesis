...
def get_article_files(metadata):
    ...
    # Construct an URL to fetch article files using article persistent IDs
    url_constructed= f"{api_url}/datasets/export?exporter=dataverse_json&persistentId={quote(persistent_id)}"
    # debug: [requests.get(url) for i in range(200)]
    
    response = requests.get(url=url_constructed, headers=header_name)
    # debug: print(f"Response status:< {response.status_code}")
    ...
    if response.status_code == 200:
        ...
        # in ['dataFile'] relevant infos : 'filename' und 'filesize'
    else:
    ...
    for attempt in range(retries):
        ...
        for index, metadata in enumerate(metadata_list):#[:10]:
            try:
                ...
                # if metadata contains data, then get file information and add to dictionary
                if len(data)>0: 
                    single_data_dict = {}
